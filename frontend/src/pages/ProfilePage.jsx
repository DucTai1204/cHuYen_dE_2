        ...
        <div className="fade-up" style={{ maxWidth: 840, margin: '0 auto', padding: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem', borderBottom: '1px solid #e2e8f0', paddingBottom: '1rem' }}>
                ...
                <div style={{
                    ...
                    {readyToWork
                        ? (isEmployer ? 'ĐANG TUYỂN DỤNG' : isSeller ? 'ĐANG NHẬN LỚP/MENTOR' : 'SẴN SÀNG ĐI LÀM')
                        : 'ĐANG TẠM ẨN'}
                </div>
            </div>
                ...
                <div style={{ background: '#fff', padding: '2rem', borderRadius: '24px', border: '1px solid #e2e8f0', boxShadow: 'var(--shadow-md)' }}>
                    <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                            <div>
                                <label style={{ display: 'block', fontSize: '.8rem', fontWeight: 800, color: '#475569', marginBottom: '.5rem', textTransform: 'uppercase', letterSpacing: '.02em' }}>
                                    {isEmployer ? 'Tên Doanh nghiệp' : isSeller ? 'Tên Thương hiệu' : 'Họ và tên'}
                                </label>
                            ...
                            <div>
                                <label style={{ display: 'block', fontSize: '.8rem', fontWeight: 800, color: '#475569', marginBottom: '.5rem', textTransform: 'uppercase', letterSpacing: '.02em' }}>
                                    {isEmployer ? 'Lĩnh vực hoạt động' : isSeller ? 'Chuyên môn giảng dạy' : 'Kỹ năng sở hữu'}
                                </label>
                                <input value={kyNang} onChange={e => setKyNang(e.target.value)} placeholder={isEmployer ? "VD: Giáo dục, IT, Tài chính..." : isSeller ? "VD: Thiết kế, Lập trình..." : "Java, SQL..."} style={{ width: '100%', padding: '.75rem 1rem', borderRadius: '12px', border: '2px solid #f1f5f9', outline: 'none', fontSize: '.9rem', background: '#fcfcfc', transition: 'all .25s' }} onFocus={e => e.target.style.borderColor = 'var(--primary)'} onBlur={e => e.target.style.borderColor = '#f1f5f9'} />
                            </div>
                        </div>

                        <div>
                            <label style={{ display: 'block', fontSize: '.8rem', fontWeight: 800, color: '#475569', marginBottom: '.5rem', textTransform: 'uppercase', letterSpacing: '.02em' }}>
                                {isEmployer ? 'Mô tả Doanh nghiệp' : isSeller ? 'Tiểu sử / Giới thiệu khóa học' : 'Giới thiệu bản thân (Bio)'}
                            </label>
                            <textarea value={bio} onChange={e => setBio(e.target.value)} placeholder={isEmployer ? "Nhập thông tin giới thiệu công ty..." : "Hãy chia sẻ một chút về bạn..."} style={{ width: '100%', padding: '.75rem 1rem', borderRadius: '12px', border: '2px solid #f1f5f9', outline: 'none', minHeight: 100, fontSize: '.9rem', background: '#fcfcfc', transition: 'all .25s', resize: 'vertical' }} onFocus={e => e.target.style.borderColor = 'var(--primary)'} onBlur={e => e.target.style.borderColor = '#f1f5f9'} />
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', padding: '1rem 1.25rem', background: 'var(--primary-light)', borderRadius: '16px', border: '1px solid var(--border)' }}>
                            <div style={{ flex: 1 }}>
                                <div style={{ fontSize: '.85rem', fontWeight: 800, color: 'var(--secondary-dark)' }}>
                                    {isEmployer ? 'Trạng thái Tuyển dụng' : isSeller ? 'Mở cửa hợp tác' : 'Trạng thái kết nối'}
                                </div>
                                <div style={{ fontSize: '.7rem', color: 'var(--secondary)', opacity: 0.8 }}>
                                    {isEmployer ? 'Hiển thị công ty trên bảng tin ứng viên.' : isSeller ? 'Cho phép đề xuất mở khóa học inhouse.' : 'Cho phép người khác tìm thấy bạn qua kỹ năng.'}
                                </div>
                            </div>
                            <button
